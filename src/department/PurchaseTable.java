...
import java.sql.*;
class PurchaseTable extends JFrame implements ActionListener
	{
	 JButton b1;
	 int count,i=0;
	...
	//		 Connection con=DriverManager.getConnection("jdbc:odbc:Emp1");
			 ...
			 String s[]={"ItemCategory","Item","Supplier","Date","Units","Price/Unit","AmountPayable"};
			 String s1[][]=new String[count][7];
			 PreparedStatement ps1=con.prepareStatement("select * from Purchase where Status=?");
				ps1.setString(1,"Purchase");
			 ...
			 while(rs1.next())
				{
				 ...
				 s1[i][3]=rs1.getString(5);
				 s1[i][4]=rs1.getString(6);
				 s1[i][5]=rs1.getString(7);
				 s1[i][6]=rs1.getString(8);
				 i++;
				}
			...
			{
			 System.out.println("kkkkkkkkkkkkkkkk"+ee);
			}
		}
